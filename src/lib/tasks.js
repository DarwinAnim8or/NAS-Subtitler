...
function createTaskService(ctx) {
   ...
   * - 从队列中移除该文件的待处理任务
   * - 若该文件对应的任务正在处理，则请求取消
   * - 将匹配的排队任务标记为已停止，并短暂保留后移除
   */
  async function setIdleForFile(dirRel, name, reason) {
    const dirRelNorm = String(dirRel || '').split('/').join('/');
    const safeName = path.basename(String(name || ''));
    const msg = reason || '手动设为不执行';
    let removedQueued = false;
    let canceledProcessing = false;
    try {
      // 从队列中移除匹配的待处理任务
      for (let i = 0; i < taskQueue.length; i++) {
        const job = taskQueue[i];
        if (!job) continue;
        const rel = path.relative(MOUNT_DIR, job.baseDir || '').split(path.sep).join('/');
        if (rel === dirRelNorm && job.safeName === safeName) {
          try { console.info('[tasks] remove queued job by file', { dirRel: dirRelNorm, name: safeName, taskId: job.taskId }); } catch (_) {}
          // 标记 activeTasks 中对应项为已停止
          const t = activeTasks.get(job.taskId);
          if (t) {
            setTaskState(job.taskId, { status: 'stopped', stage: '已停止', progress: 0, message: msg });
            try { setTimeout(() => { try { activeTasks.delete(job.taskId); } catch (_) {} }, 5000); } catch (_) {}
          }
          // 移除队列项
    ...
    try {
      // 标记排队中的匹配任务为已停止（即使队列中未找到，也覆盖 activeTasks 的排队态）
      for (const [id, t] of activeTasks.entries()) {
        if (!t) continue;
        const rel = String(t.dirRel || '').split('/').join('/');
        if (rel === dirRelNorm && t.name === safeName && t.status === 'queued') {
          try { console.info('[tasks] mark queued task stopped by file', { id, dirRel: dirRelNorm, name: safeName }); } catch (_) {}
          setTaskState(id, { status: 'stopped', stage: '已停止', progress: 0, message: msg });
          try { setTimeout(() => { try { activeTasks.delete(id); } catch (_) {} }, 5000); } catch (_) {}
          removedQueued = true;
        }
      }
    } catch (_) {}
    ...
    try {
      ...
      for (const [id, t] of activeTasks.entries()) {
        if (!t) continue;
        const rel = String(t.dirRel || '').split('/').join('/');
        if (rel === dirRelNorm && t.name === safeName && t.status === 'processing') {
          try { console.info('[tasks] request cancel processing by file', { id, dirRel: dirRelNorm, name: safeName }); } catch (_) {}
          await cancelTask(id, msg);
          canceledProcessing = true;
          break;
        }
      }
    } catch (_) {}

    return { ok: true, removedQueued, canceledProcessing };
  }
  ...
  async function startNextTask() {
    ...
    if (!Array.isArray(taskQueue) || taskQueue.length === 0) return;

    // 根据 sequence 优先级选择下一个任务（序号越大优先级越高）
    const jobs = taskQueue.slice();
    const sep = path.sep;
    const orConds = [];
    const rels = [];
    for (const j of jobs) {
      try {
        const dirRel = path.relative(MOUNT_DIR, j.baseDir).split(sep).join('/');
        rels.push(dirRel);
        orConds.push({ dirRel, name: j.safeName });
      } catch (_) {
        rels.push('');
        orConds.push({ dirRel: '', name: j.safeName });
      }
    ...
    try {
      ...
      seqMap = new Map(recs.map((r) => [`${r.dirRel}||${r.name}`, Number(r.sequence) || 0]));
    } catch (_) { }
    ...
    for (let i = 0; i < jobs.length; i++) {
      const j = jobs[i];
      const dirRel = rels[i] || '';
      const key = `${dirRel}||${j.safeName}`;
      const seq = seqMap.get(key);
      const s = (seq == null ? 0 : Number(seq) || 0);
      if (s > maxSeq) { maxSeq = s; maxIdx = i; }
    }

    const job = taskQueue.splice(maxIdx, 1)[0];
    if (!job) return;
    ...
    try {
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        try {
          ...
          segPaths[i] = null;
          try { console.warn('[tasks] segment cut failed, skip', { i, err: (e && (e.message || String(e))) }); } catch (_) {}
        }
      }
      // 分割完成后删除完整音频，确保后续仅基于分段文件进行识别
      ...
      try { if (audioPath && fs.existsSync(audioPath)) fs.unlinkSync(audioPath); } catch (_) {}
      // 额外清理：VAD 临时 WAV 以及可能的压缩中间文件，确保任何原因取消都不残留音频文件
      try {
        if (audioPath) {
          const tmpWav = audioPath.replace(/\.[^.]+$/, '_16k.wav');
          try { if (fs.existsSync(tmpWav)) fs.unlinkSync(tmpWav); } catch (_) {}
          const dir = path.dirname(audioPath);
          const base = path.basename(audioPath, path.extname(audioPath));
          const cand1 = path.join(dir, `${base}.asr.16k.m4a`);
          const cand2 = path.join(dir, `${base}.asr.16k.32k.m4a`);
          try { if (fs.existsSync(cand1)) fs.unlinkSync(cand1); } catch (_) {}
          try { if (fs.existsSync(cand2)) fs.unlinkSync(cand2); } catch (_) {}
        }
      } catch (_) {}
